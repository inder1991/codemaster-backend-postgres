/**
 * Workspace-side `.codemaster.yaml` reader (Sprint 25 / A-7-config-wireup + T-6b). Reads
 * `<workspace>/.codemaster.yaml` from the cloned review workspace and returns the WHOLE
 ...
 *
 * Branch 7 is FULL-defaults, NOT a partial merge: `CodemasterConfigV1.model_validate` rejects the WHOLE
 * document on any single invalid field (e.g. `max_findings_per_file: 9999` > 100) — there is no
 * per-field salvage. The Zod `.parse()` mirrors that exactly: any `ZodError` collapses to full defaults.
 * Branch 7 ALSO covers a non-mapping top-level (a YAML list or scalar): `.strict()` Zod object `.parse()`
 ...
 *     analogue; the legacy unsafe loader was removed) — no arbitrary-object construction.
 ...
 *     PyYAML `safe_load("")` returns `None`. Testing `data === null || data === undefined` ensures
 *     BOTH the empty-string (`undefined`) and the whitespace/comment-only/`null`-literal (`null`)
 *     cases land on the empty-file → defaults branch.
 ...
 *
 * The OTel `record_config_malformed{reason}` counter is not yet ported (separate observability
 * sub-project); each fail-open branch is annotated with the reason label so the metric wire-up is a
 ...
 *
 * FAIL-OPEN: any error returns `CodemasterConfigV1.parse({})` (full defaults). NEVER throws. Synchronous
 * (50 KiB cap; safe YAML load; whole-config-or-defaults validation). This bare-config shape is the
 * parity surface (config.parity.test.ts); the status-carrying variant below (M6) wraps it.
 */
 ...
 * customer's settings. Each malformed branch also emits a WARN as a structured log
...
export function loadRepoConfigWithStatus(workspace: string): LoadRepoConfigResultV1 {
  ...
  // (→ defaults, no metric); any other stat failure is malformed/io_error.
  let size: number;
  try {
    const st = statSync(yamlPath);
    if (!st.isFile()) {
      return absent(); // not a regular file → defaults (no metric)
    }
    size = st.size;
  } catch (err) {
    ...
    return malformed("io_error"); // stat()-failure branch (reason=io_error)
  }
  ...
  // Branch 6: empty document → defaults, STATUS 'valid' (no metric — an
  ...
  // throws a `ZodError` → FULL defaults (no partial merge). A valid document returns the validated,
  // default-filled config.
...
/** The absent-file result (defaults, no notice, no warn — the metric-free branch). */
...
function malformed(reason: ConfigMalformedReasonV1): LoadRepoConfigResultV1 {
  // The WARN ("config malformed; using defaults") is a structured log line; the OTel
...
/**
 * The full-defaults config. Zod fills every `.default(...)` from the empty object — the result the
 * fail-open branches return.
 */

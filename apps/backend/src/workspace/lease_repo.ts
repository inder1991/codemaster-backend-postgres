/**
 * LeaseRepo — 1:1 TypeScript/Kysely port of the frozen Python spine adapter
 * `vendor/codemaster-py/codemaster/workspace/_lease_repo.py` (Phase 6 / Task 7).
 *
 * Thin CRUD-only adapter over `core.workspace_leases`. Methods (1:1 with the Python class):
 *
 ...
 *                         TS clock seam here on purpose (the wall-clock value is the DB's, mirroring
 *                         the frozen Python which also uses `clock_timestamp()`, NOT an injected clock).
 *
 * ## Transaction discipline (1:1 with the Python)
 *
 ...
 * the SAME transaction — see {@link transitionLease}). State transitions themselves live in
 * `./transition.ts` (NOT here), exactly as the Python keeps `_transition` separate from `_lease_repo`.
 *
 ...
 * `// tenant:exempt reason=PK/unique-lookup follow_up=...` marker mirroring the frozen Python's
 * by-PK access (the same idiom the gate's marker escape-hatch sanctions for PK lookups).
 */

import { type Kysely, sql, type Transaction } from "kysely";

/**
 * Full `core.workspace_leases` row shape (the columns a `SELECT *` returns). 1:1 with the Python
 * `RowMapping` the repo's read methods return — callers index by column name. `snake_case` keys
 * because they are the literal DB column names asyncpg/pg/Kysely return; not renamed to camelCase so
 * the row is a faithful mirror of the table.
 */
...
/** Arguments for {@link LeaseRepo.insert}. 1:1 with the Python `insert` keyword-only signature. */
 ...
 * (over the shared ADR-0062 pool). NO clock is held: 1:1 with the frozen Python `LeaseRepo.__init__`,
 * which takes only the session — the heartbeat path uses the DB `clock_timestamp()` (statement-time)
 * rather than any injected clock (see the module docstring + {@link touchHeartbeat}).
 */
export class LeaseRepo {
  ...
  /**
   * Insert a new ALLOCATED lease row (1:1 with Python `insert`).
   *
   ...
   * DEFAULTs (`'ALLOCATED'` / `now()` / `now()`) exactly as the Python does.
   */
  ...
  /**
   * SELECT * by primary key (1:1 with Python `get_by_id`). Returns the full row, or `undefined` when
  ...
  /**
   * SELECT * by `run_id` with state in the active set (1:1 with Python `find_active_by_run`).
   *
  ...
  /**
   * Bump `heartbeat_at` for an ALLOCATED lease (1:1 with Python `touch_heartbeat`).
   *
   ...
   * a DB-side clock, outside the TS clock seam's scope (1:1 with the frozen Python).
   */
    ...
    const result = await sql`
      UPDATE core.workspace_leases
       ...
       WHERE workspace_id = ${workspaceId}
         AND state = 'ALLOCATED'
    `.execute(this.db);
    // Kysely surfaces affected-row count on `numAffectedRows` (a bigint). The Python reads
    // `result.rowcount > 0`; the TS analogue compares the bigint to 0n.

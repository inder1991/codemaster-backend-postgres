// AnnRetriever — dense retriever: embed the query (or use a caller-supplied override), delegate to an {@link AnnPort}
...
// OTel histogram emission is deferred (observability module not yet ported); the timing seam is intact.
//
// ── Purpose (W1.3 — RL-appendix embed-mode) ──
// HARDENING DIVERGENCE: the original code used "review_query" in ann_retriever but "in_repo_doc" in
// embed_query.py — two different purposes for the SAME query, so a chunk whose memoized embed failed
// got a different query vector than its siblings. Both paths now share the ONE QUERY_EMBED_PURPOSE +
// the flag-gated Qwen query-instruction seam (retrieval/query_embed.ts).
...
export class AnnRetriever {
  ...
  public constructor({ port, embeddings, modelName, clock, minSimilarity }: AnnRetrieverOptions) {
    ...
    // WallClock() default keeps zero-arg compat.
...
/** The degraded-empty envelope (fail-open path). */

// match_specificity — Stage 1.5 of the retrieval pipeline (between SQL candidate gathering and
// Stage-2 sorting): score how PRECISELY a chunk's labels match the PR's effective labels, plus the
// bucket rendered in the confluence prompt frame (Sub-spec B T16). Pure; weights verbatim from
// spec §3.5 lines 768-776, thresholds from spec §3.6 line 988.
//
// W1.3 (RH8): the original code shipped only the BUCKET half — every producer hardcoded
// `match_specificity_score = 0`, so the floors sort collapsed to age-only and the prompt attribute was
// a constant "baseline". `computeMatchSpecificity` is now WIRED:
// `PostgresConfluenceRetrieval.search` computes it per row from the caller's effective_labels.

/**
 * Per-namespace weights (spec §3.5 line 768-776). A chunk tagged only `default` matches every PR
 * equally; framework/topic-precise tags rank above it. The bare `default` label has no namespace
...
/**
 * Sum of {@link NAMESPACE_WEIGHTS} over `chunkLabels ∩ effectiveLabels`. The bare `default` label
 * contributes the baseline weight; unknown namespaces contribute 0 — they don't poison the score but

// Bm25Retriever — lexical retriever over the in-repo doc index. Delegates to a {@link Bm25Port} (Postgres `ts_rank_cd`
...
// gate-sanctioned seam), NOT a raw `performance.now()`. WallClock() default keeps zero-arg compat.
// The timing seam is intact but OTel histogram emission is deferred (not yet ported).
...
export class Bm25Retriever {
  ...
  public async retrieve(query: KnowledgeQueryV1): Promise<RetrievedKnowledgeV1> {
    ...
    // NOT forwarded — the legacy retriever always runs the default active-only path.

 ...
 *
 * codemaster ships its OWN opinionated Ruff + ESLint baselines. The runners pass these via `--config`
 * so the source repo's own `pyproject.toml` / `eslint.config.*` can never win over the bot's
 * baseline (repos may ADD rules, never WEAKEN the baseline).
 *
 ...
 *
 * Build note: `tsc` emits only `.js`; the `.toml` / `.mjs` assets must be copied into
 * `dist/apps/backend/src/config/static_analysis/` by the build step. This module only resolves the
 * path; under vitest / tsx the `src/` copy is resolved directly.
 */

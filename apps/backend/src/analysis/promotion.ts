/**
 * Linter-finding promotion primitives.
 *
 ...
 *
 * (Stages 2/3 — the Haiku curator + linter↔LLM dedup — live in their own modules.)
 */
...
export function filterToChangedLines(
  ...
  for (const f of findings) {
    ...
    // an attacker-controlled object path); the `hasOwnProperty` guard is a safe own-key presence check.

/**
 * Unified-diff range parser — extracts post-image {@link HunkRange} tuples (1-based inclusive) from
 * a GitHub PR file's `patch`
 ...
 * this file". A plain `Error` with the same "malformed hunk header" message prefix.
 */
 ...
 * boundary can convert it to "no ranges for this file".
 */
export function parseUnifiedDiffRanges(patch: string): Array<HunkRange> {
  ...
  for (const rawLine of patch.split("\n")) {
    ...
    if (match === null || match.groups === undefined) {
      // The repr-quoting via JSON.stringify matches the Python message format.

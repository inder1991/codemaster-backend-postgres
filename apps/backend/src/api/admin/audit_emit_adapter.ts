// AuditEmitPort Postgres adapter (W4.7 / EH7).
//
...
// before/after) over the core pool — audit.audit_events shares the core DSN (same-TX audit runs on
// the repo's session, so audit must be reachable on that connection).
//
// Locked invariants:

/**
 * `delete_review_placeholder` activity — tears down the PR conversation-tab placeholder comment posted by
 * `post_review_placeholder` after the
 ...
 * matches and the activity no-ops.
 *
 ...
 * module's import graph. The marker string is the contract between the two activities; the marker-lockstep
 * unit test pins them so neither can drift.
 *
 * ## DI idiom
 *
 * A pure inner {@link doDeletePlaceholder} takes injected deps + a thin Temporal wrapper
 * {@link deleteReviewPlaceholder} constructs the production GitHub client + audit-emit closure.
 */
...
/**
 * Delete the placeholder issue comment(s) (best-effort, never raises):
 *
 ...
 * pool for `dsn` per deletion and emit `REVIEW_PLACEHOLDER_DELETED` inside it.
 */
 ...
 * {@link GitHubApiReviewClient} (Vault token provider → GitHubApiClient → wrapped client) and delegates to
 * {@link doDeletePlaceholder} with the production audit-emit closure. NEVER raises (best-effort): even
 * env-resolution faults are swallowed so a misconfigured pod cannot fail the pipeline.
 */

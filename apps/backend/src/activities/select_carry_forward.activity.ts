 ...
 * ## Rule
 *
 *   - A finding `(file, start_line, end_line)` carries forward IFF its line range does NOT intersect
 *     any change in `changed_line_ranges[file]`.
 ...
 *   - A file PRESENT in `changed_line_ranges` with an EMPTY range array is treated as no-change for
 *     that file (chunks skipped, findings carried).
 *
 * Intersection is INCLUSIVE on both ends (`a_start <= b_end && a_end >= b_start`), so an
 * exact-boundary touch (e.g. finding [10,20] vs change [20,30]) counts as overlap.
 *
 * Input/output ORDER is preserved: `carried` follows `parent_findings` order; `to_review` follows
 * `current_chunks` order (single forward pass).
 *
 * ## Typed-input envelope — CLAUDE.md invariant 11 / ADR-0047 closure
 *
 * The single positional input is the {@link SelectCarryForwardInputV1} envelope, introduced during the
 * port.
 *
 * ## Runtime context
 *
 * Activities run in the NORMAL Node runtime — NOT the workflow V8-isolate sandbox. S22.DM.18
 * (consulting `core.review_findings` for prior matches) stays DEFERRED to Sprint 23+.
 */
...
/**
 * Inclusive overlap of two closed line ranges [aStart, aEnd] and [bStart, bEnd].
...
/**
 * True iff ANY change range for `file` overlaps [startLine, endLine].
...
/**
 * The pure carried-vs-to_review partition. Exported so the Tier-1 parity oracle can drive the exact
 * selection the activity runs. Returns the {@link CarryForwardSelectionV1} envelope. PURE +
 * SYNCHRONOUS — no clock, random, DB, or LLM.
 */
 ...
 * partition. Async to match the Temporal activity contract; the selection itself is pure +
 * synchronous via {@link doSelectCarryForward}.
 */

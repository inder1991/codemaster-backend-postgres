/**
 * `persistReviewWalkthrough` activity — upserts the review's structured `WalkthroughV1` into
 ...
 * fail-open is the WORKFLOW's responsibility (the stage_outcome wrapper); the activity itself simply
 * persists or raises.
 *
 * The Temporal DataConverter validates the payload through the Zod contract on the wire, so the activity
 * body never re-validates a raw dict. A malformed payload is rejected at the boundary.
 *
 ...
 * `now()` and the conflict branch sets `updated_at = now()` in SQL (see the repo).
 *
 * ## DSN
 *
 * The Postgres DSN is read from `CODEMASTER_PG_CORE_DSN` (the canonical core-store env var).
 * {@link ReviewWalkthroughsRepo.fromDsn} routes it through the ADR-0062 process-shared single pool per
 * DSN — the activity does NOT open its own pool.
 *
 * ## Idempotency + tenancy
 *
 ...
 * `CODEMASTER_PG_CORE_DSN` DSN, then delegates to `upsert` with the three fields the repo expects

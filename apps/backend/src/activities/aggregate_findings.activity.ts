/**
 * `aggregateFindings` activity — Phase-2.1 first core-loop activity. Chains the three aggregation
 * stages (scope-consistency → exact-dedup → semantic-merge → rank+cap) into one entry point and
 * returns an `AggregatedFindingsV1` envelope with per-stage statistics.
 *
 * ## Typed-input envelope — CLAUDE.md invariant 11 / ADR-0047 closure
 *
 * The single positional input is the {@link AggregateFindingsInputV1} envelope (findings +
 * policy_revision + schema_version), introduced during the port.
 *
 * ## Runtime context
 *
 * Activities run in the NORMAL Node runtime — NOT the workflow V8-isolate sandbox. The aggregation core
 * is PURE + DETERMINISTIC (no clock, no random, no DB), which is why this activity touches no Postgres.
 *
 ...
 * real embedder the semantic stage greedily merges same-file near-duplicates (cosine ≥ 0.92), so
 * `semantic_merged` is non-zero. When NO embedder is provided the stage takes the fail-open skip path
 * (`after_semantic = after_exact`, `semantic_merged = 0`). See aggregation_semantic.ts for the merge.
 */
...
/**
 * The `_do_aggregate` pipeline (stage order + stat accumulation):
 *
 ...
 * Exported so the Tier-1 parity oracle can drive the same pipeline the activity runs. The optional
 ...
 * registry wires today; production threads a real embedder via {@link AggregateFindingsActivity}.
 */
...
/**
 * Bound-method holder for the aggregate_findings activity. The worker bootstrap constructs it with
 * the resolved platform {@link EmbeddingsPort} and registers its `aggregateFindings` bound method,
 * so the semantic stage runs the REAL Qwen cosine-merge:
 *

...
-- node-pg-migrate's own pgmigrations_id_seq). The pre-fusion migrations are preserved in git history.
--
-- SEED DATA — note (2026-06-14): the SCHEMA remains semantic-diff-identical to the pre-fusion migrations,
-- but the dev-incidental seed rows the source dump captured were intentionally REMOVED (3 core.llm_models,
-- 3 core.llm_purpose_model, 4 core.flags_archive_0090) — they were a dev DB's state, not product defaults,
-- and must not ship to customers. The remaining seeds (cost-cap flags, platform_config, embedder runtime
-- state, global_config, the __platform_sentinel__ installation) ARE product defaults. See
-- docs/plans/2026-06-14-baseline-seed-cleanup.md. Still up-only; if ever regenerated by the recipe above,
-- the source DB must EXCLUDE these dev rows.
--
--

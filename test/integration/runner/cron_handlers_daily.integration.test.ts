...
//   (3) STARTUP (the 5-cron chain): ensureScheduledJobs seeds ALL 5 core.scheduled_jobs rows
//       (2 interval + 3 daily-cron); pollAndEnqueue after advancing a FakeClock past the due instant
...
//
// Plus the pure (no-DB) registry-shape checks: the 5-entry CRON_SCHEDULES literal, and that every
...
describeDb("cron_handlers — daily crons on the background-jobs platform (Phase 3b W3b.2)", () => {
  ...
  it("(3) STARTUP: ensureScheduledJobs seeds ALL 5 rows; a due poll enqueues them; cycles dispatch them; daily rows hold for their wall instants", async () => {
    const t0 = new Date("2026-06-10T00:00:00.000Z");
    const fake = new FakeClock({ now: t0 });
    await ensureScheduledJobs(db, fake);

    // ALL 5 rows seeded (ORDER BY schedule_id) — 2 interval (W3b.1) + 2 daily-cron (W3b.2) + the
    // run_id_retention daily cron (W3d.1).
    ...
    // Advance the FakeClock past the due instant → ONE poll enqueues ALL 5 (dedup_key = schedule_id).
    fake.advance({ seconds: 1 });                                          // t1 = 00:00:01Z
    const repo = new BackgroundJobsRepo(db);
    expect(await pollAndEnqueue({ repo, db, clock: fake })).toBe(5);
    ...
    // The background cycles dispatch ALL 5 through the registry to 'done' (WallClock composition —
    ...
    // client is never built; the retire/delete sweeps are idempotent cross-tenant scans).
    const handles = buildBackgroundRunner({ db, clock: new WallClock(), config: TEST_CONFIG });
    ...
    expect((await handles.runOneCycle()).outcome).toBe("idle");            // exactly 5 — nothing left
    ...
    // advancing strictly-after to tomorrow's 03:00. Drain the 4 ready jobs first so the interval
    // dedup keys are free.
    for (let i = 0; i < 4; i += 1) {
    ...
    expect(ready0300.rows.map((j) => j.job_type)).toEqual([
      "mutex_janitor", "review_run_reaper", "run_id_retention",
    ]);
...
describe("CRON_SCHEDULES (Phase 3b W3b.2 + Phase 3d W3d.1 entries)", () => {
  it("carries the 5 entries: the 2 W3b.1 intervals + the 2 daily 02:00 crons + run_id_retention at 03:00 UTC", () => {

...
describe("PostReviewPlaceholderInput parity (Pydantic ↔ Zod)", () => {
  it("validates + dumps a fully-specified payload identically", async () => {
    ...
    expect(canonicalize(PostReviewPlaceholderInput.parse(VALID))).toBe(r.out);
  }, 30_000);
  ...
  it("applies the same default (schema_version=1) when omitted", async () => {
    ...
    expect(r.ok, r.err).toBe(true);
    expect(canonicalize(PostReviewPlaceholderInput.parse(payload))).toBe(r.out);
  }, 30_000);
  ...
  it("both REJECT schema_version != 1 (Literal[1])", async () => {
    ...
    expect(() => PostReviewPlaceholderInput.parse(bad)).toThrow();
  }, 30_000);

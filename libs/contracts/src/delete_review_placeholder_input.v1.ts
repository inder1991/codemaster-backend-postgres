...
// but the contract is kept as a DISTINCT model — the two `BaseModel`s are separate classes so a future
// divergence on either side does not silently couple them.
//
...
export const DeleteReviewPlaceholderInput = z
  .object({
    ...
    // dispatch threads the id explicitly. TS-only field, symmetric with the placeholder.

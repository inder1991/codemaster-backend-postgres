...
// `pr_number: int (ge=1)`); the port therefore uses z.number().int() WITHOUT a lower bound.
export const PostReviewPlaceholderInput = z
  .object({
    ...
    // KEY required at construction so the dispatch threads the id explicitly. TS-only field, not in the Python contract.
